...
@section('content')
    <main class='py-6 md:py-10 px-4 md:px-6'>
        <div class="w-full max-w-350 mx-auto py-6 md:py-10 flex flex-col justify-center space-y-8 md:space-y-12">
            <div class='flex flex-col md:flex-row justify-between items-start md:items-center gap-6 w-full'>
                ...
                <div class="flex flex-col items-stretch md:items-end gap-3 md:gap-6 w-full md:w-auto">
                    <a href="{{ route("auto.change", ["id" => $budget->id]) }}" class="text-butter bg-secondary px-5 py-2 font-bold rounded-2xl inline-block text-center hover:text-secondary hover:bg-butter transition duration-700">Change my Budget</a>
                    <a href="{{ route("auto.history", ["budget" => $budget->id]) }}" class="text-butter bg-secondary px-5 py-2 font-bold rounded-2xl inline-block text-center hover:text-secondary hover:bg-butter transition duration-700">Budget History</a>
    ...
    <x-reset-carryover-popup :prompt="$resetCarryover" :action="route('auto.resetCarryover')" />
