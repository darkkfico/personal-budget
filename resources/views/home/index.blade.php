...
@section('header')
    <header class="md:h-screen h-full md:py-0 relative">
        <nav class="w-full flex justify-between items-center mx-auto py-5 md:py-8 px-4 md:px-10 shadow-b-secondary shadow-xl gap-2">
            <div id="logo" class="shrink-0">
                <h1 class="text-butter font-extrabold text-2xl md:text-3xl">Budgetly<span class="text-accent">.</span></h1>
            </div>
            ...
            <div class="space-x-2 md:space-x-3 flex justify-end items-center shrink-0">
                {{-- @if (Auth::check())
                    <a href='{{ route("home") }}'
                ...
                @else --}}
                    <a href='{{ route("auth.register") }}'
                        class='relative text-butter bg-secondary px-3 md:px-5 py-2 text-sm md:text-base font-bold rounded-2xl inline-block hover:text-secondary  hover:bg-butter transition duration-700'>Register</a>
                    <a href='{{ route("auth.login") }}'
                        class='relative text-secondary bg-butter px-3 md:px-5 py-2 text-sm md:text-base font-bold rounded-2xl inline-block hover:text-butter  hover:bg-secondary transition duration-700'>Log
                        In</a>
                {{-- @endif --}}
                <div class="md:hidden inline-block">
                    <i class="fa-solid fa-bars text-2xl font-bold text-butter"></i>
                    <div id="menu" class="hidden w-[min(20rem,100vw)] h-screen bg-secondary fixed z-100 top-0 right-0 py-10 px-6 space-y-10 transition duration-500">
        ...
        <section
            class="w-full px-4 md:px-6 h-full flex flex-col md:flex-row space-y-10 justify-between items-center max-w-250 mx-auto md:-mt-10 py-10 md:py-0">
            <div id="left" class="md:w-[50%] w-full space-y-8 md:space-y-14 min-w-0">
                <h2
                ...
                <h1 class="text-4xl md:text-7xl text-butter font-extrabold leading-tight">Organize Your Monthly Budget And Save Money</h1>
    ...
    <main id="main-section">
        <section class="h-full bg-secondary my-auto pb-28 px-6 md:px-0">
            <div class="w-full max-w-250 flex flex-col md:flex-row md:justify-between items-center mx-auto space-y-16">
                ...
                <div id="left" class="relative md:w-1/2 w-full flex flex-col items-center justify-center min-w-0">
                    <div id="title" class="space-y-4 w-full">
                        <h1 class="text-butter text-3xl md:text-4xl font-bold">Can't Organize Your Monthly Budget</h1>
                        <p class="text-butter text-lg md:text-xl">Try this app and see the results</p>
                    </div>
                    <div class="md:hidden w-full max-w-sm mt-8 space-y-4">
                        <img src="{{ asset('images/main-img.jpg') }}" alt="" class="w-full border-2 border-white object-cover">
                        <img src="{{ asset('images/main2-img.webp') }}" alt="" class="w-full border-2 border-white object-cover">
                    </div>
                    <div class="hidden md:block absolute border-2 border-white w-[400px] -mt-[50px] md:-ml-[350px] z-20 overflow-hidden">
                        <img src="{{ asset('images/main-img.jpg') }}" alt="" class="w-full h-full object-cover">
                    </div>
                    <img src="{{ asset('images/main2-img.webp') }}" alt=""
                        class="hidden md:block relative border-2 border-white w-[400px] mt-[300px] z-10 ml-[100px] object-cover">
                </div>
                <div id="right" class="md:w-1/2 w-full md:px-20 space-y-14 flex py-10 min-w-0">
                    <div
                        class="relative before:absolute before:bg-butter before:w-1 before:h-[80%] md:before:-left-[62px] before:-left-[37px] space-y-10 ml-8 md:ml-15 px-4 md:px-6 w-full">

                        @for ($i = 1; $i <= 4; $i++)
                            @if ($i % 2 == 0)
                                <div class='relative bg-secondary rounded-2xl shadow-md shadow-yellow-100 px-5 py-2'>
                                    <div class='absolute -left-[72px] md:-left-[88px] -top-5 '>
                                        <p
                ...
                <div id="left" class="bg-secondary md:w-1/3 w-full md:rounded-l-2xl rounded-t-2xl px-8 py-5 h-full">
                    ...
                    <div class="space-y-6 mt-5">
                        <div class="flex justify-start items-center gap-2">
                            ...
                            <p class="text-butter text-base md:text-xl break-all">example@gmail.com</p>
                        </div>
                        <div class="flex justify-start items-center gap-2">
                        ...
                        <div class="flex justify-start items-center gap-2">
                            ...
                            <p class="text-butter text-base md:text-xl break-all">budgetly_monthly_budget</p>
                        </div>
                    </div>
                </div>
